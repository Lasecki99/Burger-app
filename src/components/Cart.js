...
const cart = (props) => {
    ...
    const isSomethingInCart = props.globalPrice > 0 ?
        <Link
            onClick={() => document.querySelector('div.summary').style.display = 'none'}
            className="continue" to='/order'>Continue</Link> :
        <p className="empty">There are currently no orders in your shopping cart.</p>
    return (
        <BrowserRouter>
